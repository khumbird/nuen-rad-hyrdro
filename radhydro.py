...
def calc_grid_area_vol(N,Rmax,geometry,Nt):
    r=np.zeros((N+1,Nt)).astype('float64')
    for j in range(0,Nt):
        r[:,j]=np.linspace(0,Rmax,N+1)
    A=np.zeros((N+1,Nt)).astype('float64')
    V=np.zeros((N,Nt)).astype('float64')
...
def get_abs_opacity(T,k,Tbl,Tbr):
    ...
    #k1=1;k2=1;k3=1;
    n=1
    kappa=np.zeros(len(T)).astype('float64')
...
def artif_viscosity(P,gamma,u,r,rho,k):
    cQ=(gamma+1.0)/4.0
    Q=np.zeros(len(P)).astype('float64')
    dr=np.zeros(len(P)).astype('float64')
    du=np.zeros(len(P)).astype('float64')
    Rm=np.zeros(len(P)).astype('float64')
    Rp=np.zeros(len(P)).astype('float64')
    rhom=np.zeros(len(P)).astype('float64')
    rhop=np.zeros(len(P)).astype('float64')    
    csm=np.zeros(len(P)).astype('float64')
    csp=np.zeros(len(P)).astype('float64')      
...
def predictor_rad_E(RadE,A,m,r,u,rho,T,P,Cv,TbL,TbR,dt_prev,dt,k,gamma):
    ...
    nu=np.zeros(len(T)).astype('float64')
    xi=np.zeros(len(T)).astype('float64')
    F0p=np.zeros(len(T)).astype('float64')
    F0m=np.zeros(len(T)).astype('float64')
    for i in range(0,len(T)):
        nu[i]=(dtk*kappaA[i]*c*2*a*T[i,k-1]**3)/(Cv[i]+dtk*kappaA[i]*c*2.0*a*T[i,k-1]**3)
        xi[i]=-(P[i,k-1]+QP[i])*(A[i+1,k-1]*0.5*(u[i+1,k-1]+u[i+1,k])- A[i,k-1]*0.5*(u[i,k-1]+u[i,k])) 
    for i in range(1,len(T)-1):        
        F0p[i]=-2.0*c/(3.0*(0.5*(rho[i,k]+rho[i,k-1])*(0.5*(r[i+1,k]+r[i+1,k-1]-r[i,k]-r[i,k-1]))*kappaTm[i]+0.5*(rho[i+1,k]+rho[i+1,k-1])*(0.5*(r[i+2,k]+r[i+2,k-1]-r[i+1,k]-r[i+1,k-1]))*kappaTm[i+1])) #not sure about opacity eval        
        F0m[i]=-2.0*c/(3.0*(0.5*(rho[i-1,k]+rho[i-1,k-1])*(0.5*(r[i,k]+r[i,k-1]-r[i-1,k]-r[i-1,k-1]))*kappaTp[i]+0.5*(rho[i,k]+rho[i,k-1])*(0.5*(r[i+1,k]+r[i+1,k-1]-r[i,k]-r[i,k-1]))*kappaTp[i])) #not sure about opacity eval
    C=np.zeros((len(RadE),len(RadE))).astype('float64')
    Q=np.zeros(len(RadE)).astype('float64')
    
    F0p[0]=-2.0*c/(3.0*(0.5*(rho[0,k]+rho[0,k-1])*(0.5*(r[1,k]+r[1,k-1]-r[0,k]-r[0,k-1]))*kappaTm[0]+0.5*(rho[i,k]+rho[1,k-1])*(0.5*(r[2,k]+r[2,k-1]-r[1,k]-r[1,k-1]))*kappaTm[1]))
    F0m[0]=-2*c/(3*0.5*(rho[0,k]+rho[0,k-1])*0.5*(r[1,k]+r[1,k-1]-r[0,k]-r[0,k-1])*kappaTm[0]+4.0)
    C[0,0]=m[0,k]/(dtk*rho[0,k])+0.5*m[0,k]*kappaA[0]*c*(1.0-nu[0])-0.5*(A[1,k-1]+A[1,k])*0.5*F0p[0]-0.5*(A[0,k-1]+A[0,k])*0.5*F0m[0]
    ...
    Q[0]=nu[0]*xi[0]+m[0,k]*kappaA[0]*c*(1.0-nu[0])*(a*T[0,k-1]**4-0.5*RadE[0,k-1])-(1./3.0)*RadE[0,k-1]*(A[1,k-1]*0.5*(u[1,k-1]+u[1,k])-A[0,k-1]*0.5*(u[0,k-1]+u[0,k]))+ \
         0.5*(A[0,k-1]+A[0,k])*0.5*F0m[0]*(RadE[0,k-1]-a*TbL**4)-0.5*(A[1,k-1]+A[1,k])*0.5*F0p[0]*(RadE[1,k-1]-RadE[0,k-1])+m[0,k]*RadE[0,k-1]/(dtk*rho[0,k-1])

    F0p[-1]=-2*c/(3*0.5*(rho[-1,k]+rho[-1,k-1])*0.5*(r[-1,k]+r[-1,k-1]-r[-2,k]-r[-2,k-1])*kappaTm[-1]+4.0)
    F0m[-1]=-2.0*c/(3.0*(0.5*(rho[-2,k]+rho[-2,k-1])*(0.5*(r[-2,k]+r[-2,k-1]-r[-3,k]-r[-3,k-1]))*kappaTp[-2]+0.5*(rho[-1,k]+rho[-1,k-1])*(0.5*(r[-1,k]+r[-1,k-1]-r[-2,k]-r[-2,k-1]))*kappaTp[-1]))
    C[-1,-1]=m[-1,k]/(dtk*rho[-1,k])+0.5*m[-1,k]*kappaA[-1]*c*(1.0-nu[-1])-0.5*(A[-2,k-1]+A[-2,k])*0.5*F0m[-1]-0.5*(A[-1,k-1]+A[-1,k])*0.5*F0p[-1]
    C[-1,-2]=0.5*F0m[-1]*0.5*(A[-2,k-1]+A[-2,k])
    Q[-1]=nu[-1]*xi[-1]+m[-1,k]*kappaA[-1]*c*(1.0-nu[-1])*(a*T[-1,k-1]**4-0.5*RadE[-1,k-1])-(1./3.0)*RadE[-1,k-1]*(-A[-2,k-1]*0.5*(u[-2,k-1]+u[-2,k])+A[-1,k-1]*0.5*(u[-1,k-1]+u[-1,k]))+ \
         0.5*(A[-1,k-1]+A[-1,k])*0.5*F0p[-1]*(-a*TbR**4+RadE[-1,k-1])-0.5*(A[-2,k-1]+A[-2,k])*0.5*F0m[-1]*(RadE[-2,k-1]-RadE[-1,k-1])+m[-1,k]*RadE[-1,k-1]/(dtk*rho[-1,k-1])
...
def corrector_velocity(u,dt_prev,dt,m,A,r,rho,P,RadE,k,PbR,PbL,TbR,TbL,T,gamma,Tp):  #i is i+1/2, k is k+1/2
    mm=np.zeros(len(u)).astype('float64')
    for i in range(1,len(u)-1):
        mm[i]=0.5*(m[i-1,0]+m[i,0])
    mm[0]=0.5*m[0,0]
    mm[-1]=0.5*m[-1,0]
    Ppk=0.5*(P[:,k-1]+P[:,k])#get_Ppk(P,k)
    Apk=get_Apk(A,k)   
    RadEpk=0.5*(RadE[:,k-1]+RadE[:,k]) #get_RadEpk(RadE,k)
    dtk=0.5*(dt_prev+dt)    
    ...
    u[0,k]=b[0];u[-1,k]=b[1]
    for i in range(1,len(u)-1):
        u[i,k]=u[i,k-1]-dtk*(Apk[i]/mm[i])*((Ppk[i]+QPk[i])+(1./3.)*(RadEpk[i]-RadEpk[i-1])-(Ppk[i-1]+QPkm1[i-1]))       
    return(u)
...
def corrector_rad_E(RadE,A,m,r,u,rho,T,P,Cv,TbL,TbR,dt_prev,dt,k,gamma,Ap,Tp):
    ...
    nu=np.zeros(len(P)).astype('float64')
    xi=np.zeros(len(P)).astype('float64')
    F0p=np.zeros(len(T)).astype('float64')
    F0m=np.zeros(len(T)).astype('float64')
    Ppk=get_Ppk(Pp,k)
    Apk=get_Apk(Ap,k)
    for i in range(0,len(P)):
        nu[i]=(dtk*kappaA[i]*c*2.0*a*T[i,k]**3)/(Cv[i]+dtk*kappaA[i]*c*2*a*T[i,k]**3)
        xi[i]=-(Ppk[i]+QP[i])*(Apk[i+1]*0.5*(u[i+1,k-1]+u[i+1,k])- Apk[i]*0.5*(u[i,k-1]+u[i,k]))-(m[i,k]/dtk)*(e[i,k]-e[i,k-1])   

    for i in range(1,len(T)-1):        
        F0p[i]=-2.0*c/(3.0*(0.5*(rho[i,k]+rho[i,k-1])*(0.5*(r[i+1,k]+r[i+1,k-1]-r[i,k]-r[i,k-1]))*kappaT[i]+0.5*(rho[i+1,k]+rho[i+1,k-1])*(0.5*(r[i+2,k]+r[i+2,k-1]-r[i+1,k]-r[i+1,k-1]))*kappaT[i+1])) #not sure about opacity eval
        F0m[i]=-2.0*c/(3.0*(0.5*(rho[i-1,k]+rho[i-1,k-1])*(0.5*(r[i,k]+r[i,k-1]-r[i-1,k]-r[i-1,k-1]))*kappaT[i-1]+0.5*(rho[i,k]+rho[i,k-1])*(0.5*(r[i+1,k]+r[i+1,k-1]-r[i,k]-r[i,k-1]))*kappaT[i])) #not sure about opacity eval
    C=np.zeros((len(RadE),len(RadE)))
    Q=np.zeros(len(RadE))
    
    F0p[0]=-2.0*c/(3.0*(0.5*(rho[0,k]+rho[0,k-1])*(0.5*(r[1,k]+r[1,k-1]-r[0,k]-r[0,k-1]))*kappaT[0]+0.5*(rho[i,k]+rho[1,k-1])*(0.5*(r[2,k]+r[2,k-1]-r[1,k]-r[1,k-1]))*kappaT[1]))
    ...
    F0m[-1]=-2.0*c/(3.0*(0.5*(rho[-2,k]+rho[-2,k-1])*(0.5*(r[-2,k]+r[-2,k-1]-r[-3,k]-r[-3,k-1]))*kappaT[-2]+0.5*(rho[-1,k]+rho[-1,k-1])*(0.5*(r[-1,k]+r[-1,k-1]-r[-2,k]-r[-2,k-1]))*kappaT[-1]))
    C[-1,-1]=m[-1,k]/(dtk*rho[-1,k])+0.5*m[-1,k]*kappaA[-1]*c*(1.0-nu[-1])-0.5*(A[-2,k-1]+A[-2,k])*0.5*F0m[-1]-0.5*(A[-1,k-1]+A[-1,k])*0.5*F0p[-1]
    C[-1,-2]=0.5*F0m[-1]*0.5*(A[-2,k-1]+A[-2,k])
...
def corrector_internale(e,RadE,T,P,A,u,r,dt_prev,dt,k,Cv,m,gamma,rho,Tp):
    ...
    for i in range(0,len(P)):
        xi[i]=-(Ppk[i]+QP[i])*(Apk[i+1]*0.5*(u[i+1,k-1]+u[i+1,k])- Apk[i]*0.5*(u[i,k-1]+u[i,k]))-(m[i,k]/dtk)*(e[i,k]-e[i,k-1])   
    for i in range(0,len(e)):
        e[i,k]=e[i,k]+(dtk*Cv[i]*(m[i,k]*c*kappaA[i]*(0.5*(RadE[i,k]+RadE[i,k-1])-a*0.5*(T[i,k-1]**4+T[i,k]**4))+xi[i]))/(m[i,k]*Cv[i]+dtk*m[i,k]*kappaA[i]*c*2*a*(T[i,k]**3))
    return(e)     
    

def compute_initialenergy(m,u,e,RadE,rho):
    sum3=0.0;sum4=0.0;sum5=0.0;
    mm0=np.zeros(len(u)).astype('float64')
    for i in range(1,len(u)-1):
...
def compute_advleak(P,gamma,u,r,rho,k,RadE,A,dt,dt_prev,Tp,Pp,Ap,RadEp,Pbl,Pbr,rp,rhop):
    ...
    lina=(Apk[0]*((1./3.)*RadE12+Pbl)*0.5*(u[0,k]+u[0,k-1])-Apk[-1]*((1./3.)*RadEN12+Pbr)*0.5*(u[-1,k]+u[-1,k-1]))*dtk
    return(lina)
    
def compute_radleak(P,gamma,u,r,rho,k,RadE,A,dt,dt_prev,Tp,Pp,Ap,RadEp,Pbl,Pbr):
    kappaT=get_tot_opacity(T,k,TbL,TbR)
    a=0.01372
    c=299.792 
    dtk=0.5*(dt_prev+dt)    
    F0L=-2*c/(3*0.5*(rho[0,k]+rho[0,k-1])*0.5*(r[1,k]+r[1,k-1]-r[0,k]-r[0,k-1])*kappaT[0]+4.0)*(0.5*(RadE[0,k]+RadE[0,k-1])-a*TbL**4)
    F0R=-2*c/(3*0.5*(rho[-1,k]+rho[-1,k-1])*0.5*(r[-1,k]+r[-1,k-1]-r[-2,k]-r[-2,k-1])*kappaT[-1]+4.0)*(a*TbR**4-0.5*(RadE[-1,k]+RadE[-1,k-1]))
...
c=299.7920
N=int(Rmax/dx)
rho=np.ones((N,Nt)).astype('float64')
rho=rho*rho0  
m=np.ones((N,Nt)).astype('float64') 
Cv=np.ones(N).astype('float64')
Cv=Cv*Cv0
u=np.ones((N+1,Nt)).astype('float64')
u=u*u0
T=np.ones((N,Nt)).astype('float64')
T=T*T0
r,A,V=calc_grid_area_vol(N,Rmax,geometry,Nt)  
RadE=np.ones((N,Nt)).astype('float64')
RadE=RadE*a*(T0**4)
for i in range(0, N):
    for j in range(0,Nt):
        m[i,j]=rho[i,j]*V[i,j]
dt_vect=np.zeros(Nt).astype('float64')
e=np.zeros((N,Nt)).astype('float64')
for i in range(0,N):e[i,:]=T[i,0]*Cv[i]    
P=np.ones((N,Nt))
for i in range(0,N):P[i,:]=(gamma-1)*rho[i,:]*e[i,:]
lin=0.0;
lin2=0.0
bal=np.zeros(Nt).astype('float64')
kev=np.zeros(Nt).astype('float64')
radev=np.zeros(Nt).astype('float64')
iev=np.zeros(Nt).astype('float64')
totv=np.zeros(Nt).astype('float64')
linv=np.zeros(Nt).astype('float64')
lin2v=np.zeros(Nt).astype('float64')
loutv=np.zeros(Nt).astype('float64') 
...
for k in range(1,Nt):
    ...
    lin2=compute_radleak(P,gamma,u,r,rho,k,RadE,A,dt,dt_prev,Tp,Pp,Ap,RadEp,PbL,PbR) #####
    ...
    totv[k]=iev[k]+kev[k]+radev[k]+lin2v[k]+linv[k]
    bal[k]=1.0-(iev[k]+kev[k]+radev[k]+lin2v[k]+linv[k])/Ienergy
    print(1.0-(iev[k]+kev[k]+radev[k]+lin2v[k]+linv[k])/(iev[k-1]+kev[k-1]+radev[k-1]+lin2v[k-1]+linv[k-1]))
   # print(1.0-(iev[k]+kev[k]+radev[k]+lin2v[k]+linv[k])/Ienergy)
